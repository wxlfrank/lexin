...
function DictItem(word, explain) {
    var words = word.split("Engelsk ");
    this.word = {
        orginal: word,
        real: words[0],
    };
    if (words.length > 1)
        this.word.eng = words[1];
    ...
    this.query_suggest = function (word, results, success) {
        ...
        perform(word, function (res) {
            for (var index = 0; index < res.rows.length; ++index) {
                ...
                var word = getField(row, "word");
                // word = word.replace(/Engelsk /g, "<br>En");
                results.push(new DictItem(word, getField(row, "explain")));
            }
            if (!!success)
                success();
        }, function (table) {
